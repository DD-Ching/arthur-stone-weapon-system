...
##
## Spawns Arthur and one dummy IN FRONT, holds the facing, flings a momentum swing
## (the head whips from behind, around, to the front), and asserts the swing:
##   - launched the dummy (knockback)
##   - cost stamina
##   - emitted hit_landed
##
## Arthur's own _physics_process is disabled so his mouse-aim (the headless mouse
## sits at the origin) and swing-lunge don't drift the test; we drive the weapon
## directly. try_spend_stamina / lunge are plain method calls, unaffected.
##
...
func _ready() -> void:
	...
	dummy.global_position = Vector2(72, 0)  # in FRONT, in the flung swing's path
	arthur.set_physics_process(false)       # drive the weapon directly; no mouse hijack / lunge drift
...
func _physics_process(_delta: float) -> void:
	_frame += 1
	arthur.weapon.set_aim_target(0.0)   # hold facing +X; the head flings to the front
	match _frame:
		2:
