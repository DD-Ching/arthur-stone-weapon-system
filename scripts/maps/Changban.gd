...
# 蜀 Shu rally banners + war drums crown the escort's rally points; shovable crates/rocks and
...
func _build_decor() -> void:
	...
	super._build_decor()
	for side in [-1.0, 1.0]:
		# 蜀 Shu rally banners crowning each wing — a far one at the northern muster, a near one
		# at the southern rally point behind the ward (their green reads as "our" people fleeing).
		_banner(Vector2(520.0 * side, -260.0), "shu", 78.0)
		_banner(Vector2(480.0 * side, 300.0), "shu", 84.0)
		# A 蜀 Shu war-drum on each wing, beating the escort onward.
...
## Drop a 蜀/魏/吳/neutral faction standard, tinting + sizing it per placement.
...
## Drop a 蜀 Shu war drum at a spot (green rim = our camp).
...
func _draw() -> void:
	...
	# A 蜀 Shu-green rally ring at the ward's muster point (the escort's goal, behind Arthur).
