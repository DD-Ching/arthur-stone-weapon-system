...
func _draw() -> void:
	...
	# A Saxon banner above the gate (enemy side) and a Camelot banner below (our side).
	var saxon := Color(0.30, 0.52, 0.95)
	var camelot := Color(0.36, 0.78, 0.42)
	draw_rect(Rect2(-12.0, gate_y - 64.0, 24.0, 40.0), saxon)
	draw_rect(Rect2(-12.0, 360.0, 24.0, 40.0), camelot)
