...
@export var wave_clear_threshold := 5      ## spawn the next wave once raiders drop to this
...
func _build_context() -> Dictionary:
	var alive := get_tree().get_nodes_in_group("targets").size()
	var ward_ok: bool = (_ward == null) or (is_instance_valid(_ward) and not ("_dead" in _ward and _ward._dead))
	var ctx := {
